...
{
	public ReinforcedDoorItem(Settings properties)
	{
		super(properties);
	}
	...
	public ActionResult useOnBlock(PlayerEntity player, World world, BlockPos pos, ItemStack stack, Direction facing, double hitX, double hitY, double hitZ, ItemUsageContext ctx)
	{
		...
		if (player.canPlaceOn(pos, facing, stack) && BlockUtils.isSideSolid(world, pos.down(), Direction.UP))
		{
			...
			if(!placeDoor(world, pos, angleFacing, SCContent.REINFORCED_DOOR, flag))
				return ActionResult.FAIL;
			...
			{
				((OwnableTileEntity) world.getBlockEntity(pos)).getOwner().set(player.getGameProfile().getId().toString(), player.getName());
				((OwnableTileEntity) world.getBlockEntity(pos.up())).getOwner().set(player.getGameProfile().getId().toString(), player.getName());
			}
	...
	public boolean placeDoor(World world, BlockPos pos, Direction facing, Block door, boolean isRightHinge) //naming might not be entirely correct, but it's giving a rough idea
	{
		...
		BlockState state = door.getDefaultState().with(ReinforcedDoorBlock.FACING, facing).with(ReinforcedDoorBlock.HINGE, isRightHinge ? DoorHinge.RIGHT : DoorHinge.LEFT).with(ReinforcedDoorBlock.OPEN, isAnyPowered);
