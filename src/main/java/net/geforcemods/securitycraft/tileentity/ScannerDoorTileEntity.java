...
public class ScannerDoorTileEntity extends CustomizableTileEntity
{
	...
	public void entityViewed(LivingEntity entity)
	{
		...
		if(!world.isClient && upperState.get(DoorBlock.HALF) == DoubleBlockHalf.UPPER && !EntityUtils.isInvisible(entity))
		{
			...
			{
				PlayerUtils.sendMessageToPlayer(player, ClientUtils.localize(SCContent.SCANNER_DOOR_ITEM.getTranslationKey()), ClientUtils.localize("messages.securitycraft:retinalScanner.notOwner", getOwner().getName()), Formatting.RED);
				return;
			}
			...
			if(open && sendMessage.get())
				PlayerUtils.sendMessageToPlayer(player, ClientUtils.localize(SCContent.SCANNER_DOOR_ITEM.getTranslationKey()), ClientUtils.localize("messages.securitycraft:retinalScanner.hello", player.getName()), Formatting.GREEN);
		}
	}
