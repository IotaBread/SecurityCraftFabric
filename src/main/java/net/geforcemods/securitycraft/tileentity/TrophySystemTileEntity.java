...
public class TrophySystemTileEntity extends OwnableTileEntity implements Tickable {
...
//	}
	...
	private void destroyTarget() {
		...
		if(!world.isClient)
			world.createExplosion(null, entityBeingTargeted.getX(), entityBeingTargeted.getY(), entityBeingTargeted.getZ(), 0.1F, Explosion.DestructionType.NONE);
	...
	private Entity getTarget() {
		...
		Box area = new Box(pos).expand(RANGE, RANGE, RANGE);

		// Add all arrows and fireballs to the targets list. Could always add more
		// projectile types if we think of any
		potentialTargets.addAll(world.getEntitiesByClass(PersistentProjectileEntity.class, area, e -> !(e instanceof TridentEntity))); //ignore tridents
		potentialTargets.addAll(world.getNonSpectatingEntities(ExplosiveProjectileEntity.class, area));
		potentialTargets.addAll(world.getNonSpectatingEntities(ShulkerBulletEntity.class, area));
	...
	public UUID getShooterUUID(Entity entity) {
		if(entity instanceof PersistentProjectileEntity && ((PersistentProjectileEntity) entity).getOwner() != null) //getShooter
			return ((PersistentProjectileEntity) entity).getOwner().getUuid(); //getShooter
		else if(entity instanceof FireballEntity && ((FireballEntity) entity).getOwner() != null) //getShooter
			return ((FireballEntity) entity).getOwner().getUuid(); //getShooter
		else
			return null;
	}
